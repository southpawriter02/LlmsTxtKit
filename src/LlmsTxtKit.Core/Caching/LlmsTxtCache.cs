...
public sealed class LlmsTxtCache
{
    ...
    /// <summary>Logger for cache operations: hits, misses, evictions, SWR decisions.</summary>
    private readonly ILogger<LlmsTxtCache> _logger;
    ...
    /// </param>
    /// <param name="logger">
    /// Optional logger for cache diagnostic output. Logs hits, misses, evictions,
    /// stale-while-revalidate decisions, and backing store interactions.
    /// </param>
    public LlmsTxtCache(CacheOptions? options = null, ILogger<LlmsTxtCache>? logger = null)
    {
        _options = options ?? new CacheOptions();
        _logger = logger ?? NullLogger<LlmsTxtCache>.Instance;
    }
    ...
    public async Task<CacheEntry?> GetAsync(string domain, CancellationToken cancellationToken = default)
    {
        ...
        if (_store.TryGetValue(key, out var entry))
        {
            ...
            if (result != null)
                _logger.LogDebug("Cache hit (in-memory) for domain={Domain}, expired={Expired}", domain, entry.IsExpired());
            else
                _logger.LogDebug("Cache expired (SWR disabled) for domain={Domain}", domain);
            return result;
        }
        ...
        {
            _logger.LogDebug("Cache miss (in-memory) for domain={Domain}, checking backing store", domain);
            var loaded = await _options.BackingStore.LoadAsync(key, cancellationToken).ConfigureAwait(false);
            if (loaded != null)
            {
                // Promote to in-memory cache
                _store.TryAdd(key, loaded);
                _logger.LogDebug("Loaded from backing store and promoted for domain={Domain}", domain);
                return ProcessEntry(loaded);
            }
        }

        _logger.LogDebug("Cache miss for domain={Domain}", domain);
        return null;
    }
    ...
    public async Task SetAsync(string domain, CacheEntry entry, CancellationToken cancellationToken = default)
    {
        ...
        _store[key] = entry;
        _logger.LogDebug("Cache set for domain={Domain}, expiresAt={ExpiresAt}, storeSize={Size}",
            domain, entry.ExpiresAt, _store.Count);
        ...
        if (_options.BackingStore != null)
        {
            ...
            _logger.LogDebug("Persisted to backing store for domain={Domain}", domain);
        }
        ...
        lock (_evictionLock)
        {
            ...
            while (_store.Count > _options.MaxEntries)
            {
                ...
                if (oldestKey != null)
                {
                    ...
                    _logger.LogDebug("LRU eviction: removed domain={Domain}, lastAccess={LastAccess}, storeSize={Size}",
                        oldestKey, oldestAccess, _store.Count);
                }
                else
                {
