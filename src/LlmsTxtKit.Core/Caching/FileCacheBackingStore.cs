...
public sealed class FileCacheBackingStore : ICacheBackingStore
{
    ...
    /// <summary>Logger for file I/O operations and error diagnostics.</summary>
    private readonly ILogger<FileCacheBackingStore> _logger;
    ...
    /// </param>
    /// <param name="logger">
    /// Optional logger for file I/O diagnostics. Logs save/load operations,
    /// corrupted file warnings, and I/O errors.
    /// </param>
    public FileCacheBackingStore(string? cacheDirectory = null, ILogger<FileCacheBackingStore>? logger = null)
    {
        _logger = logger ?? NullLogger<FileCacheBackingStore>.Instance;
    ...
    public async Task SaveAsync(string key, CacheEntry entry, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Saved cache entry to file: key={Key}, path={Path}, size={Size} bytes",
            key, filePath, json.Length);
    }
    ...
    public async Task<CacheEntry?> LoadAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        catch (JsonException ex)
        {
            ...
            _logger.LogWarning("Corrupted cache file for key={Key}, path={Path}: {Error}",
                key, filePath, ex.Message);
            return null;
        }
        catch (IOException ex)
        {
            // File was deleted or is being written by another process — treat as a miss.
            _logger.LogWarning("I/O error reading cache file for key={Key}, path={Path}: {Error}",
                key, filePath, ex.Message);
            return null;
        }
    }
